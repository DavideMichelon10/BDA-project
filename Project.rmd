...
spiegare dataset come dalla ta
...
```
Dire che non ce ne sono sempre 22 perchè non tutti arrivano in fondo
...
data { 
  ...
  int<lower=0> J_teams; #number of teams
...
model {
  //Priors
  for (j in 1:J_riders){
      mu_riders[j] ~ normal(0, 100);
  }
  for (i in 1:J_teams){
      mu_teams[i] ~ normal(0, 100);
  }
  ...
  //Likelihood
  //for (i in 1:J_teams){
  //  y_teams[,i] ~ beta(mu_teams[i]*sigma, (1-mu_teams[i])*sigma);
  //}
  //for (j in 1:J_riders){
  //  y_riders[,j] ~ beta(mu_riders[j]*sigma, (1-mu_riders[j])*sigma);
  //}
  
  for (j in 1:J_riders){
    for (i in 1:J_teams){
      mu = inv_logit(mu_teams[i] + mu_riders[j]);
      y_teams[,i] ~ beta(mu*sigma, (1-mu)*sigma);
    }
  }
  
  for (i in 1:J_teams){
    for (j in 1:J_riders){
      mu = inv_logit(mu_teams[j] + mu_riders[i]);
      y_riders[,j] ~ beta(mu*sigma, (1-mu)*sigma);
    }
  }
